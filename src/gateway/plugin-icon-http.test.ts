...
describe("Control UI plugin and catalog icon routes", () => {
  ...
  it("closes the descriptor when rejecting an empty package icon", async () => {
    ...
    // The loader uses the named node:fs export, which must follow the real close spy.
    ...
    try {
      ...
      // Observe release itself: another worker may already have reused the fd number.
