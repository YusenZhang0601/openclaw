...
describe("Control UI plugin and catalog icon routes", () => {
  ...
  it("closes the descriptor when rejecting an empty package icon", async () => {
    ...
    try {
      ...
      // Observe release itself: another worker may reuse the fd after the HTTP await.
      expect(closed).toHaveBeenCalledWith(receipt.fd);
    } finally {
      const receipt = await opened.mock.results[0]?.value;
      const wasClosed = closed.mock.calls.some(([fd]) => receipt?.ok && fd === receipt.fd);
      opened.mockRestore();
      closed.mockRestore();
      syncBuiltinESMExports();
      if (receipt?.ok && !wasClosed) {
