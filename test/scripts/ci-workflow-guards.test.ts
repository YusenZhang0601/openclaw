...
it("pins simple release admission owners before selected checkout and preserves Git contracts", () => {
  ...
  ).toBe("apps/linux/src-tauri");
  const finalizeAppImage = expectDefined(
    linuxBuildSteps.find(({ name }) => name === "Finalize AppImage"),
    "Linux AppImage finalizer step",
  );
  expect(finalizeAppImage.env ?? {}).not.toHaveProperty("TAURI_SIGNING_PRIVATE_KEY");
  expect(finalizeAppImage.env ?? {}).not.toHaveProperty("TAURI_SIGNING_PRIVATE_KEY_PASSWORD");
  ...
  expect(signAppImage.run).toContain("signer sign");
